  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div
        ...
        style={{ backgroundColor: '#0F1B31', border: '1px solid rgba(255,255,255,0.1)' }}
      >
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: '#94A3B8' }}>
            Análise comparativa e calibração de indicadores
          </p>
        </div>
        <select
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {[
          ...
          },
          {
            ...
            value:
              entries.length > 0
                ? `${(entries.reduce((s, e) => s + e.qa, 0) / entries.length).toFixed(1)}%`
                : '—',
            color: '#22C55E',
          ...
          },
          {
            ...
            value:
              entries.length > 0
                ? `${(entries.reduce((s, e) => s + e.iepc, 0) / entries.length).toFixed(1)}%`
                : '—',
            color: '#06B6D4',
      ...
      {!loading && chartData.length > 0 && (
        ...
        >
          <h3 className="text-sm font-semibold text-white mb-4">
            Distribuição QA & IEPC — {selectedPeriodo}
          </h3>
              ...
              {entries.map((e, i) => (
                ...
                >
                  ...
                  <td
                    ...
                    style={{ color: e.qa >= 85 ? '#22C55E' : e.qa >= 70 ? '#F59E0B' : '#EF4444' }}
                  >
                    {e.qa.toFixed(1)}%
                  </td>
                  <td
                    className="px-4 py-3 font-semibold"
                    style={{
                      color: e.iepc >= 85 ? '#22C55E' : e.iepc >= 70 ? '#F59E0B' : '#EF4444',
                    }}
                  >
                    {e.iepc.toFixed(1)}%
                  </td>
                        ...
                        backgroundColor:
                          e.qa >= 85
                            ...
                            : e.qa >= 70
                              ? 'rgba(245,158,11,0.1)'
                              : 'rgba(239,68,68,0.1)',
                        color: e.qa >= 85 ? '#22C55E' : e.qa >= 70 ? '#F59E0B' : '#EF4444',
                      }}
                    >
