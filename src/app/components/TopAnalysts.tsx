...
export default function TopAnalysts() {
  ...
  const loadData = async () => {
    ...
    if (scores.length === 0) { setHasData(false); return; }
  ...
  if (!hasData) {
    return (
      <div className="rounded-xl p-6 h-full flex flex-col items-center justify-center text-center"
        ...
        <p className="text-xs" style={{ color: '#8B949E' }}>Importe dados para ver os analistas em destaque</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="mb-4">
        <h3 className="font-display text-base font-semibold text-white" style={{ fontFamily: "'Playfair Display', serif" }}>Destaques do Ciclo</h3>
        <p className="text-xs mt-0.5" style={{ color: '#8B949E' }}>Top 3 analistas e Squad Ouro</p>
      </div>
      {/* Top 3 analysts */}
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs truncate" style={{ color: '#8B949E' }}>{analyst.squad}</p>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="text-sm font-bold text-white metric-value">{analyst.nota_final_qa.toFixed(1)}</p>
      ...
      {(squadOuroQA || squadOuroIEPC) && (
        <>
          ...
          <div>
            <p className="text-xs font-medium uppercase tracking-wide mb-3" style={{ color: '#8B949E' }}>Squad Ouro</p>
            <div className="grid grid-cols-2 gap-2">
              {squadOuroQA && (
                <div className="p-3 rounded-xl text-center" style={{ backgroundColor: 'rgba(234,179,8,0.08)', border: '1px solid rgba(234,179,8,0.2)' }}>
                  <Trophy size={16} className="mx-auto mb-1.5" style={{ color: '#EAB308' }} />
                  <p className="text-xs font-semibold text-white leading-tight">{squadOuroQA.squad}</p>
                  <p className="text-lg font-bold metric-value" style={{ color: '#EAB308' }}>{squadOuroQA.avgQA.toFixed(1)}</p>
                  <p className="text-xs" style={{ color: '#8B949E' }}>QA Média</p>
                </div>
              )}
              {squadOuroIEPC && (
                <div className="p-3 rounded-xl text-center" style={{ backgroundColor: 'rgba(43,79,129,0.12)', border: '1px solid rgba(43,79,129,0.3)' }}>
                  <Award size={16} className="mx-auto mb-1.5" style={{ color: '#60A5FA' }} />
                  <p className="text-xs font-semibold text-white leading-tight">{squadOuroIEPC.squad}</p>
                  <p className="text-lg font-bold metric-value" style={{ color: '#60A5FA' }}>{squadOuroIEPC.avgIEPC.toFixed(1)}</p>
                  <p className="text-xs" style={{ color: '#8B949E' }}>IEPC Médio</p>
                </div>
              )}
            </div>
          </div>
