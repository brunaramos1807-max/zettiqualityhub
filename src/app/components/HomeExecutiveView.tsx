...
const NC_DETAILS = [
  { code: 'NC-1', name: 'Postura e Ética Profissional', definicao: 'Comportamento inadequado, falta de ética ou postura profissional comprometida.', severidade: 'Variável', impacto: 'Risco reputacional e cultural', exemplos: ['Linguagem inapropriada', 'Descaso com o cliente', 'Comportamento antiético'], reincidencia: 'Requer PDI obrigatório na reincidência' },
  { code: 'NC-2', name: 'Acuracidade e Rigor Técnico', definicao: 'Informação técnica incorreta ou solução inadequada fornecida ao cliente.', severidade: 'Variável', impacto: 'Retrabalho e insatisfação do cliente', exemplos: ['Diagnóstico errado', 'Solução incorreta', 'Informação desatualizada'], reincidencia: 'Gera PDI obrigatório na 3ª ocorrência' },
  { code: 'NC-3', name: 'Conformidade de Registro e Rastreabilidade', definicao: 'Falha no registro, documentação ou rastreabilidade da demanda.', severidade: 'Variável', impacto: 'Comprometimento da auditoria e governança', exemplos: ['Atendimento não registrado', 'Documentação incompleta', 'Protocolo não gerado'], reincidencia: 'Revisão de processo obrigatória' },
  { code: 'NC-4', name: 'Integridade do Fluxo Operacional', definicao: 'Desvio do fluxo operacional estabelecido ou não seguimento de protocolo.', severidade: 'Variável', impacto: 'Inconsistência operacional e risco de SLA', exemplos: ['Pular etapa do processo', 'Encaminhamento incorreto', 'Protocolo não seguido'], reincidencia: 'Treinamento obrigatório' },
  { code: 'NC-5', name: 'Segurança da Informação', definicao: 'Exposição ou manuseio inadequado de dados sensíveis do cliente.', severidade: 'Crítica', impacto: 'Risco legal, LGPD e reputacional', exemplos: ['Compartilhamento de dados indevido', 'Acesso não autorizado', 'Exposição de informações confidenciais'], reincidencia: 'Processo disciplinar imediato' },
];
...
export default function HomeExecutiveView() {
  const { getTeamFilter } = useSystemAuth();
  ...
  const evolutionData = filteredHistory.map(h => ({ periodo: h.periodo, qa: h.qa, iepc: h.iepc, ncs: h.ncs, elogios: h.elogios }));
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (!active || !payload?.length) return null;
    return (
      <div className="rounded-xl p-3 text-xs shadow-xl" style={{ backgroundColor: '#FFFFFF', border: '1px solid rgba(15,23,42,0.08)' }}>
  ...
  const formatDelta = (delta: number | null, suffix = '') => {
    if (delta === null) return 'Sem comparativo';
    if (delta === 0) return `Estável${suffix}`;
    return `${delta > 0 ? '+' : ''}${delta.toFixed(1)}${suffix}`;
  };

  const operationStatus = qaMedia >= 85 && iepcMedia >= 85 && totalNCs <= 5
    ? { label: 'Saúde boa', tone: '#059669', bg: '#ECFDF5', border: '#A7F3D0', summary: 'Operação em patamar saudável para o ciclo selecionado.' }
    : qaMedia < 70 || iepcMedia < 70 || totalNCs > 15
      ? { label: 'Atenção crítica', tone: '#DC2626', bg: '#FEF2F2', border: '#FECACA', summary: 'Há indicadores que exigem priorização de gestão neste ciclo.' }
      : { label: 'Atenção controlada', tone: '#D97706', bg: '#FFFBEB', border: '#FDE68A', summary: 'Operação estável, com pontos específicos para acompanhamento.' };

  const strongestSquad = squadRanking[0];
  const weakestSquad = squadRanking[squadRanking.length - 1];
  const positiveHighlights = [
    qaDelta !== null && qaDelta > 0 ? `QA evoluiu ${qaDelta.toFixed(1)} ponto(s) vs ${prevPeriod?.periodo}.` : null,
    iepcDelta !== null && iepcDelta > 0 ? `IEPC evoluiu ${iepcDelta.toFixed(1)} ponto(s) vs ${prevPeriod?.periodo}.` : null,
    ncDelta !== null && ncDelta < 0 ? `Não conformidades reduziram ${Math.abs(ncDelta)} registro(s).` : null,
    elogioDelta !== null && elogioDelta > 0 ? `Reconhecimentos aumentaram ${elogioDelta} registro(s).` : null,
    strongestSquad ? `${strongestSquad.squad} lidera o ciclo em QA (${strongestSquad.qa.toFixed(1)}).` : null,
    totalElogios > 0 ? `${totalElogios} reconhecimento(s) registrado(s) no ciclo.` : null,
  ].filter(Boolean) as string[];

  const attentionPoints = [
    qaDelta !== null && qaDelta < 0 ? `QA caiu ${Math.abs(qaDelta).toFixed(1)} ponto(s) vs ${prevPeriod?.periodo}.` : null,
    iepcDelta !== null && iepcDelta < 0 ? `IEPC caiu ${Math.abs(iepcDelta).toFixed(1)} ponto(s) vs ${prevPeriod?.periodo}.` : null,
    ncDelta !== null && ncDelta > 0 ? `NCs aumentaram ${ncDelta} registro(s) no ciclo.` : null,
    weakestSquad && weakestSquad !== strongestSquad ? `${weakestSquad.squad} requer acompanhamento em QA (${weakestSquad.qa.toFixed(1)}).` : null,
    qaStrategicPillars.length > 0 ? `Pilar QA mais frágil: ${qaStrategicPillars.slice().sort((a, b) => a.pct - b.pct)[0]?.fullName}.` : null,
    iepcStrategicPillars.length > 0 ? `Dimensão IEPC mais frágil: ${iepcStrategicPillars.slice().sort((a, b) => a.pct - b.pct)[0]?.fullName}.` : null,
  ].filter(Boolean) as string[];

  const nextActions = [
    weakestSquad ? { text: `Acompanhar ${weakestSquad.squad} e definir plano de evolução do indicador mais baixo.`, href: '/cycle-dashboard' } : null,
    totalNCs > 0 ? { text: 'Revisar não conformidades abertas e recorrências do ciclo.', href: '/nao-conformidades' } : null,
    positiveHighlights.length > 0 ? { text: 'Reconhecer publicamente os destaques positivos do ciclo.', href: '/mural-elogios' } : null,
    qaDelta !== null || iepcDelta !== null ? { text: 'Comparar tendência no analytics operacional antes da próxima decisão.', href: '/cycle-dashboard' } : null,
  ].filter(Boolean) as { text: string; href: string }[];

  const governanceAlerts = [
    { text: 'Fechamento, reabertura e correção de ciclos devem ser feitos em Gestão de Ciclos.', href: '/ciclos' },
    totalNCs > 0 ? { text: 'Há NCs que exigem acompanhamento operacional.', href: '/nao-conformidades' } : null,
  ].filter(Boolean) as { text: string; href: string }[];
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#F6F8FB' }}>
      <div className="p-6 max-w-screen-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: '#0F172A' }}>Resumo Executivo</h1>
            <p className="text-sm mt-1" style={{ color: '#64748B' }}>Saúde da operação, evolução, riscos e próximas decisões</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              {/* Ciclo filter */}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <h2 className="text-lg font-bold mb-2" style={{ color: '#0F172A' }}>Nenhum dado importado</h2>
            <p className="text-sm mb-6 max-w-sm" style={{ color: '#64748B' }}>
              O resumo executivo ficará disponível após a entrada de dados operacionais. Use a área correta de importações para alimentar o ciclo.
            </p>
            <Link href="/importacoes" className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold" style={{ backgroundColor: '#E0F2FE', color: '#0369A1', border: '1px solid #BAE6FD' }}>
              Acessar Importações <ChevronRight size={14} />
            </Link>
          </div>
        ) : (
          <div className="space-y-6">
            {/* Saúde da Operação */}
            <section className="rounded-2xl p-6" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0', boxShadow: '0 12px 30px rgba(15,23,42,0.06)' }}>
              <div className="flex items-start justify-between gap-6 mb-5">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#64748B' }}>Saúde da Operação</p>
                  <h2 className="text-2xl font-bold mt-1" style={{ color: '#0F172A' }}>{operationStatus.label}</h2>
                  <p className="text-sm mt-1" style={{ color: '#64748B' }}>{operationStatus.summary}</p>
                </div>
                <div className="px-3 py-2 rounded-xl text-sm font-semibold" style={{ color: operationStatus.tone, backgroundColor: operationStatus.bg, border: `1px solid ${operationStatus.border}` }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                {[
                  { label: 'QA geral', value: qaMedia.toFixed(1), delta: formatDelta(qaDelta), status: qaClass.label, color: qaClass.color, icon: <BarChart2 size={16} /> },
                  { label: 'IEPC geral', value: iepcMedia.toFixed(1), delta: formatDelta(iepcDelta), status: iepcClass.label, color: iepcClass.color, icon: <Star size={16} /> },
                  { label: 'Não conformidades', value: String(totalNCs), delta: ncDelta === null ? 'Sem comparativo' : ncDelta <= 0 ? `${Math.abs(ncDelta)} a menos` : `+${ncDelta}`, status: totalNCs === 0 ? 'Sem registros' : totalNCs > 10 ? 'Atenção' : 'Monitorar', color: totalNCs > 10 ? '#DC2626' : '#D97706', icon: <AlertTriangle size={16} /> },
                  { label: 'Reconhecimentos', value: String(totalElogios), delta: elogioDelta === null ? 'Sem comparativo' : elogioDelta >= 0 ? `+${elogioDelta}` : `${elogioDelta}`, status: totalElogios > 0 ? 'Evidência positiva' : 'Sem registros', color: '#059669', icon: <CheckCircle size={16} /> },
                ].map((card) => (
                  <div key={card.label} className="rounded-2xl p-4" style={{ backgroundColor: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#64748B' }}>{card.label}</span>
                      <span className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ color: card.color, backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0' }}>{card.icon}</span>
                    </div>
                    <p className="text-3xl font-bold" style={{ color: '#0F172A' }}>{card.value}</p>
                    <div className="flex items-center justify-between mt-3 text-xs">
                      <span style={{ color: card.color }}>{card.delta}</span>
                      <span style={{ color: '#64748B' }}>{card.status}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              {/* Evolução */}
              <section className="xl:col-span-2 rounded-2xl p-5" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0' }}>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-base font-bold" style={{ color: '#0F172A' }}>Evolução do Suporte</h3>
                    <p className="text-sm" style={{ color: '#64748B' }}>Tendência por ciclo de QA, IEPC, NCs e reconhecimentos.</p>
                  </div>
                  <Link href="/cycle-dashboard" className="text-sm flex items-center gap-1" style={{ color: '#0369A1' }}>Analytics <ChevronRight size={14} /></Link>
                </div>
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={evolutionData.slice(-8)} margin={{ top: 12, right: 16, left: 0, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
                      <XAxis dataKey="periodo" stroke="#64748B" fontSize={11} />
                      <YAxis yAxisId="score" stroke="#64748B" fontSize={11} domain={[0, 100]} />
                      <YAxis yAxisId="count" orientation="right" stroke="#94A3B8" fontSize={11} allowDecimals={false} />
                      <Tooltip content={<CustomTooltip />} />
                      <Line yAxisId="score" type="monotone" dataKey="qa" name="QA" stroke="#2563EB" strokeWidth={2.5} dot={<CustomDot />} />
                      <Line yAxisId="score" type="monotone" dataKey="iepc" name="IEPC" stroke="#0891B2" strokeWidth={2.5} dot={<CustomDot />} />
                      <Line yAxisId="count" type="monotone" dataKey="ncs" name="NCs" stroke="#DC2626" strokeWidth={2} dot={false} />
                      <Line yAxisId="count" type="monotone" dataKey="elogios" name="Reconhecimentos" stroke="#059669" strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </section>
              ...
              {/* Decisão */}
              <section className="rounded-2xl p-5" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0' }}>
                <h3 className="text-base font-bold" style={{ color: '#0F172A' }}>Decisão recomendada agora</h3>
                <p className="text-sm mt-1 mb-4" style={{ color: '#64748B' }}>Leitura objetiva para priorização da gestão.</p>
                <div className="space-y-3">
                  {(nextActions.length > 0 ? nextActions.slice(0, 4) : [{ text: 'Dados insuficientes para recomendação automática neste ciclo.', href: '/cycle-dashboard' }]).map((action, index) => (
                    <Link key={index} href={action.href} className="block rounded-xl p-3 transition-colors" style={{ backgroundColor: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                      <p className="text-sm font-medium" style={{ color: '#0F172A' }}>{action.text}</p>
                    </Link>
                  ))}
                </div>
              </section>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              {/* Positivos */}
              <section className="rounded-2xl p-5" style={{ backgroundColor: '#FFFFFF', border: '1px solid #DCFCE7' }}>
                <div className="flex items-center gap-2 mb-4">
                  <TrendingUp size={16} style={{ color: '#059669' }} />
                  <h3 className="text-base font-bold" style={{ color: '#0F172A' }}>Destaques positivos do ciclo</h3>
                </div>
                <div className="space-y-3">
                  {(positiveHighlights.length > 0 ? positiveHighlights.slice(0, 5) : ['Sem destaque positivo automático com os dados atuais.']).map((item, index) => (
                    <div key={index} className="rounded-xl p-3" style={{ backgroundColor: '#F0FDF4', border: '1px solid #DCFCE7' }}>
              ...
              {/* Atenção */}
              <section className="rounded-2xl p-5" style={{ backgroundColor: '#FFFFFF', border: '1px solid #FEF3C7' }}>
                <div className="flex items-center gap-2 mb-4">
                  <AlertTriangle size={16} style={{ color: '#D97706' }} />
                  <h3 className="text-base font-bold" style={{ color: '#0F172A' }}>Pontos de atenção</h3>
                </div>
                <div className="space-y-3">
                  {(attentionPoints.length > 0 ? attentionPoints.slice(0, 5) : ['Nenhum risco automático relevante identificado neste ciclo.']).map((item, index) => (
                    <div key={index} className="rounded-xl p-3" style={{ backgroundColor: '#FFFBEB', border: '1px solid #FEF3C7' }}>
                      <p className="text-sm" style={{ color: '#92400E' }}>{item}</p>
                    </div>
                  ))}
                </div>
              </section>
              ...
              {/* Governança */}
              <section className="rounded-2xl p-5" style={{ backgroundColor: '#FFFFFF', border: '1px solid #DBEAFE' }}>
                <div className="flex items-center gap-2 mb-4">
                  <Info size={16} style={{ color: '#2563EB' }} />
                  <h3 className="text-base font-bold" style={{ color: '#0F172A' }}>Alertas de governança</h3>
                </div>
                <div className="space-y-3">
                  {governanceAlerts.map((alert, index) => (
                    <Link key={index} href={alert.href} className="block rounded-xl p-3" style={{ backgroundColor: '#EFF6FF', border: '1px solid #DBEAFE' }}>
                      <p className="text-sm" style={{ color: '#1E40AF' }}>{alert.text}</p>
                    </Link>
                  ))}
                </div>
              </section>
            ...
            <section className="rounded-2xl p-5" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0' }}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-base font-bold" style={{ color: '#0F172A' }}>Squads em foco</h3>
                  <p className="text-sm" style={{ color: '#64748B' }}>Leitura rápida para coordenação e qualidade.</p>
                </div>
                <Link href="/cycle-dashboard" className="text-sm flex items-center gap-1" style={{ color: '#0369A1' }}>Ver detalhamento <ChevronRight size={14} /></Link>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
                {(squadRanking.length > 0 ? squadRanking.slice(0, 4) : [{ squad: 'Sem squad no ciclo', qa: 0 }]).map((item) => {
                  const cls = getPerformanceClass(item.qa);
                  return (
                    <div key={item.squad} className="rounded-xl p-4" style={{ backgroundColor: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                      <p className="text-sm font-semibold truncate" style={{ color: '#0F172A' }}>{item.squad}</p>
                      <p className="text-2xl font-bold mt-2" style={{ color: cls.color }}>{item.qa.toFixed(1)}</p>
                      <p className="text-xs mt-1" style={{ color: '#64748B' }}>{cls.label}</p>
                    </div>
                  );
                })}
              </div>
