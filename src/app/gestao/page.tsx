...
                { label: 'Média QA', value: analista.avgQA > 0 ? analista.avgQA.toFixed(1) : '—', color: getScoreColor(analista.avgQA) },
                { label: 'Média IEPC', value: analista.avgIEPC > 0 ? analista.avgIEPC.toFixed(1) : '—', color: getScoreColor(analista.avgIEPC) },
                { label: 'Total NCs', value: analista.totalNCs, color: analista.totalNCs > 0 ? '#EF4444' : '#22C55E' },
              ].map((kpi) => (
                <div key={kpi.label} className="text-center p-2 rounded-lg" style={{ backgroundColor: 'rgba(255,255,255,0.04)' }}>
                  <p className="text-xs mb-1" style={{ color: '#8B949E' }}>{kpi.label}</p>
...
                    <Tooltip contentStyle={{ backgroundColor: '#161B22', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8 }} labelStyle={{ color: '#fff', fontWeight: 600 }} />
...
      style={{ backgroundColor: '#0F1B31', border: `1px solid ${isBirthdayMonth ? 'rgba(245,158,11,0.3)' : 'rgba(255,255,255,0.07)'}`, boxShadow: '0 2px 12px rgba(0,0,0,0.3)' }}
      onClick={onClick}
    >
      {/* Header */}
...
          {isBirthdayMonth && <Gift size={14} style={{ color: '#F59E0B' }} title="Aniversariante do mês" />}
...
          <div className="flex items-center gap-2 text-xs" style={{ color: isBirthdayMonth ? '#F59E0B' : '#94A3B8' }}>
            <Gift size={11} />
...
        </div>
        <span className="text-xs px-2 py-0.5 rounded-full font-medium"
          style={{ backgroundColor: `${getStatusColor(analista.status)}15`, color: getStatusColor(analista.status) }}>
          {getStatusLabel(analista.status)}
        </span>
      </div>
    </div>
...
  const currentMonth = new Date().getMonth();
  const aniversariantes = analistas.filter((a) => getBirthdayMonth(a.data_nascimento) === currentMonth && a.status !== 'desligado');
  if (aniversariantes.length === 0) return null;

  return (
    <div className="p-4 rounded-2xl" style={{ backgroundColor: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)' }}>
      <div className="flex items-center gap-2 mb-3">
        <Gift size={16} style={{ color: '#F59E0B' }} />
        <h3 className="text-sm font-semibold text-white">Aniversariantes do Mês</h3>
        <span className="text-xs px-2 py-0.5 rounded-full font-bold" style={{ backgroundColor: 'rgba(245,158,11,0.2)', color: '#F59E0B' }}>
          {aniversariantes.length}
        </span>
      </div>
      <div className="flex flex-wrap gap-2">
        {aniversariantes.map((a) => (
          <div key={a.id} className="flex items-center gap-2 px-3 py-1.5 rounded-xl"
            style={{ backgroundColor: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.15)' }}>
...
              <p className="text-xs" style={{ color: '#F59E0B' }}>{formatBirthday(a.data_nascimento)}</p>
            </div>
          </div>
        ))}
      </div>
...
  return (
    <div className="p-4 rounded-2xl" style={{ backgroundColor: '#0F1B31', border: '1px solid rgba(255,255,255,0.07)' }}>
      <div className="flex items-center gap-2 mb-3">
...
            <span className="text-xs font-bold w-5 text-center" style={{ color: i === 0 ? '#F59E0B' : '#8B949E' }}>#{i + 1}</span>
...
              <p className="text-xs" style={{ color: '#8B949E' }}>{a.squad || a.equipe || '—'}</p>
            </div>
            <span className="text-xs font-medium" style={{ color: '#38BDF8' }}>{a.tempoEmpresa}</span>
          </div>
        ))}
      </div>
    </div>
...
          ? analistaScores.reduce((sum: number, s: any) => sum + (s.nota_final_qa || 0), 0) / analistaScores.length : 0;
        const avgIEPC = analistaScores.length > 0
          ? analistaScores.reduce((sum: number, s: any) => sum + (s.iepc_total || 0), 0) / analistaScores.length : 0;
...
    const currentMonth = new Date().getMonth();
    const aniversariantes = analistas.filter((a) => getBirthdayMonth(a.data_nascimento) === currentMonth && a.status !== 'desligado').length;
    const avgTempo = analistas.filter((a) => a.mesesEmpresa > 0).length > 0
      ? Math.round(analistas.filter((a) => a.mesesEmpresa > 0).reduce((s, a) => s + a.mesesEmpresa, 0) / analistas.filter((a) => a.mesesEmpresa > 0).length)
      : 0;
...
          </p>
        </div>
        <button onClick={loadData} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all hover:bg-white/5"
...
          { label: 'Aniversariantes', value: stats.aniversariantes, color: '#F59E0B', icon: <Gift size={16} /> },
          { label: 'Tempo Médio', value: stats.avgTempoLabel, color: '#A78BFA', icon: <Clock size={16} /> },
          { label: 'PDIs Ativos', value: stats.comPdi, color: '#818CF8', icon: <BookOpen size={16} /> },
          { label: 'Novos (≤3m)', value: stats.novos, color: '#34D399', icon: <UserPlus size={16} /> },
        ].map((kpi) => (
...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar analista, equipe, coordenador..."
            style={{ backgroundColor: '#0F1B31', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '0.75rem', color: '#F8FAFC', padding: '0.625rem 0.875rem 0.625rem 2.25rem', fontSize: '0.875rem', width: '100%', outline: 'none' }} />
        </div>
        {[
          { value: filterEquipe, onChange: setFilterEquipe, options: [{ value: '', label: 'Todas equipes' }, ...equipes.map((e) => ({ value: e, label: e }))] },
          { value: filterStatus, onChange: setFilterStatus, options: [{ value: '', label: 'Todos status' }, { value: 'ativo', label: 'Ativo' }, { value: 'ferias', label: 'Férias' }, { value: 'afastado', label: 'Afastado' }, { value: 'desligado', label: 'Desligado' }] },
          { value: sortBy, onChange: (v: string) => setSortBy(v as typeof sortBy), options: [{ value: 'nome', label: 'Ordenar: Nome' }, { value: 'tempo', label: 'Ordenar: Tempo Empresa' }, { value: 'aniversario', label: 'Ordenar: Aniversário' }, { value: 'pdi', label: 'Ordenar: PDI Ativo' }] },
        ].map((sel, i) => (
          <select key={i} value={sel.value} onChange={(e) => sel.onChange(e.target.value)}
            style={{ backgroundColor: '#0F1B31', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '0.75rem', color: '#F8FAFC', padding: '0.625rem 0.875rem', fontSize: '0.875rem', outline: 'none', cursor: 'pointer' }}>
...
          <p className="text-sm" style={{ color: '#94A3B8' }}>
            {analistas.length === 0 ? 'Cadastre analistas na página Analistas para visualizá-los aqui.' : 'Tente ajustar os filtros de busca.'}
          </p>
        </div>
      ) : (
...
      {selectedAnalista && (
        <AnalystDetailDrawer analista={selectedAnalista} onClose={() => setSelectedAnalista(null)} />
      )}
    </div>
  );
}
