...
function AdminDiagnosticContent() {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      if (!supabase) return;
      const [usersRes, cyclesRes, scoresRes, ncsRes, pdisRes, logsRes, permLogsRes] =
        await Promise.all([
          supabase
            .from('user_profiles')
          ...
          supabase
            .from('import_cycles')
            .select('id, periodo, status, is_closed, imported_at')
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: '#94A3B8' }}>
            Monitoramento completo do sistema Qualivisão
          </p>
        </div>
            ...
            {[
              {
                ...
                sub: `${stats.activeUsers} ativos`,
                color: '#38BDF8',
                icon: <Users size={16} />,
              },
              {
                label: 'Ciclos Importados',
              ...
              {
                label: 'Avaliações QA',
                value: stats.totalScores.toLocaleString('pt-BR'),
                sub: 'registros',
                color: '#06B6D4',
              ...
              },
              {
                ...
                value: stats.totalNCs.toLocaleString('pt-BR'),
                sub: 'registros',
                color: '#F59E0B',
              ...
              },
              {
                ...
                sub: 'últimos logs',
                color: stats.recentErrors > 0 ? '#EF4444' : '#22C55E',
                icon: <XCircle size={16} />,
              },
              {
                label: 'Status Geral',
                value: stats.recentErrors === 0 ? 'OK' : 'Atenção',
                sub: 'sistema',
                color: stats.recentErrors === 0 ? '#22C55E' : '#F59E0B',
                icon: <Activity size={16} />,
              },
            ].map((s) => (
              <div
              ...
              <table className="w-full text-xs">
                <thead>
                  ...
                  >
                    {['Data', 'Ator', 'Ação', 'Entidade', 'Detalhes'].map((h) => (
                      <th
                        key={h}
                        className="text-left py-3 px-4 font-semibold uppercase tracking-wide"
                        style={{ color: '#94A3B8' }}
                      >
                        {h}
                      </th>
                ...
                <tbody>
                  ...
                  {permissionLogs.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-8 text-center" style={{ color: '#94A3B8' }}>
                        Nenhum log de permissão registrado
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      {activeTab === 'logs' && (
        <div>
          ...
          <div className="flex gap-3 mb-4 flex-wrap">
            ...
            <span className="flex items-center text-xs" style={{ color: '#64748B' }}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                ...
                >
                  {['Usuário', 'Email', 'Cargo/Role', 'Status', 'Último Login', 'Criado em'].map(
                    (h) => (
                      <th
                        key={h}
                        className="text-left py-3 px-4 font-semibold uppercase tracking-wide"
              ...
              <tbody>
                ...
                {users.length === 0 && (
                  <tr>
                    <td colSpan={6} className="py-8 text-center" style={{ color: '#94A3B8' }}>
