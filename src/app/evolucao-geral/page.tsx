...
function EvolucaoGeralContent() {
  ...
  const [manualCycles, setManualCycles] = useState<
    { periodo: string; qa_media: number; iepc_media: number; total_ncs: number }[]
  >([]);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [scoresData, ncsData] = await Promise.all([fetchCycleScores(), fetchNCRecords()]);
  ...
  const squadSectorEntries = useMemo(
    () => manualEntries.filter((e) => e.mode === 'squad' || e.mode === 'sector'),
    [manualEntries]
  );
  ...
  const trendData = useMemo(() => {
    ...
    return Object.entries(byPeriodo)
      ...
      .map(([periodo, vals]) => ({
        ...
        iepc: vals.iepc.length
          ? +(vals.iepc.reduce((a, b) => a + b, 0) / vals.iepc.length).toFixed(2)
          : 0,
      }));
  ...
  const modeConfig = {
    analyst: {
      label: 'Por Analista',
      icon: Users,
      desc: 'Lançar resultado individual de um analista',
    },
    squad: {
      label: 'Por Equipe',
      icon: BarChart2,
      desc: 'Lançar resultado consolidado de uma equipe/squad',
    },
    sector: {
      label: 'Setor Suporte Todos',
      icon: Globe,
      desc: 'Lançar resultado geral de todo o setor de suporte',
    },
  };
  ...
  const modeBadge = (mode: string) => {
    if (mode === 'analyst')
      return { bg: 'rgba(43,79,129,0.2)', color: '#60A5FA', text: 'Analista' };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: '#8B949E' }}>
                Tendências e análise tática — {allPeriodos.length} ciclo
                {allPeriodos.length !== 1 ? 's' : ''} disponível
                {allPeriodos.length !== 1 ? 'is' : ''}
              </p>
            </div>
            ...
            <div style={cardStyle}>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs" style={{ color: '#8B949E' }}>
                  {manualEntries.length} registro{manualEntries.length !== 1 ? 's' : ''}
                </span>
              </div>
                    ...
                    {manualEntries.map((entry) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2 px-3" style={{ color: '#8B949E' }}>
                            {entry.squad}
                          </td>
                          <td
                            className="py-2 px-3 font-bold"
                            style={{ color: getScoreColor(entry.nota_final_qa) }}
                          >
                            {entry.nota_final_qa.toFixed(1)}
                          </td>
                          <td
                            className="py-2 px-3 font-bold"
                            style={{ color: getScoreColor(entry.iepc_total) }}
                          >
                            {entry.iepc_total.toFixed(1)}
                          </td>
                          <td
                            className="py-2 px-3"
                            style={{ color: entry.total_ncs > 0 ? '#EF4444' : '#22C55E' }}
                          >
                            {entry.total_ncs}
                          </td>
                          <td className="py-2 px-3">
            ...
            <div className="flex items-center justify-center py-20">
              <div className="text-center space-y-3">
                <div
                  ...
                  style={{ borderColor: '#2B4F81', borderTopColor: 'transparent' }}
                />
                    ...
                    <thead>
                      <tr>
                        ...
                        {allPeriodos.map((p) => (
                          <th
                            key={p}
                            className="text-center py-2 px-3 text-xs font-medium uppercase tracking-wide"
                            style={{ color: '#8B949E' }}
                          >
                            {p}
                          </th>
                              ...
                              <td key={p} className="py-3 px-3 text-center">
                                <div
                                  ...
                                  style={{
                                    ...
                                    color: val === 0 ? '#8B949E' : getScoreColor(val),
                                  }}
                                >
                                  {val > 0 ? val.toFixed(1) : '—'}
                                </div>
                    ...
                    <BarChart data={ncBarData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                      ...
                      {ncTypes.map((tipo, i) => (
                        <Bar
                          ...
                          dataKey={tipo}
                          stackId="a"
                          fill={NC_TYPE_COLORS[i % NC_TYPE_COLORS.length]}
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(Object.entries(modeConfig) as [EntryMode, typeof modeConfig.analyst][]).map(
                    ([key, cfg]) => {
                      const Icon = cfg.icon;
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, nota_final_qa: parseFloat(e.target.value) || 0 })
                      }
                      style={inputStyle}
                    />
                    {form.nota_final_qa > 0 && (
                      <div
                        ...
                        style={{ color: getScoreColor(form.nota_final_qa) }}
                      >
                        {form.nota_final_qa >= 85
                          ? 'Excelente'
                          : form.nota_final_qa >= 70
                            ? 'Atenção' :'Crítico'}
                      </div>
                    )}
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, iepc_total: parseFloat(e.target.value) || 0 })
                      }
                      style={inputStyle}
                    />
                    {form.iepc_total > 0 && (
                      <div
                        className="text-xs mt-1 font-semibold"
                        style={{ color: getScoreColor(form.iepc_total) }}
                      >
                        {form.iepc_total >= 85
                          ? 'Excelente'
                          : form.iepc_total >= 70
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, total_ncs: parseInt(e.target.value) || 0 })
                      }
                      style={inputStyle}
                    />
                  </div>
