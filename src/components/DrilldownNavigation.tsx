...
export type DrilldownLevel =
  'painel' | 'ciclo' | 'analista' | 'avaliacao' | 'atendimento' | 'criterios';
...
const LEVEL_ORDER: DrilldownLevel[] = [
  'painel',
  'ciclo',
  'analista',
  'avaliacao',
  ...
  return (
    <nav className="flex items-center gap-1 flex-wrap mb-4">
      {visibleLevels.map((level, idx) => {
        ...
        const label =
          level === 'ciclo' && state.ciclo
            ? state.ciclo
            : level === 'analista' && state.analista
              ? state.analista.nome.split(' ')[0]
...
export function CicloLevel({ ciclo, onSelectAnalista, onBack }: CicloLevelProps) {
  ...
  const filtered = analistas.filter(
    (a) =>
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: '#94A3B8' }}>
            {analistas.length} analistas neste ciclo
          </p>
        </div>
      </div>
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <p
                    ...
                    style={{ color: a.qa >= 85 ? '#22C55E' : a.qa >= 70 ? '#F59E0B' : '#EF4444' }}
                  >
                    {a.qa.toFixed(1)}%
                  </p>
                  <p className="text-xs" style={{ color: '#94A3B8' }}>
                ...
                <div className="text-right">
                  <p
                    className="text-xs font-semibold"
                    style={{
                      color: a.iepc >= 85 ? '#22C55E' : a.iepc >= 70 ? '#F59E0B' : '#EF4444',
                    }}
                  >
                    {a.iepc.toFixed(1)}%
                  </p>
...
export function AnalistaLevel({ analista, ciclo, onSelectAvaliacao, onBack }: AnalistaLevelProps) {
  ...
  const qaMedia =
    scores.length > 0 ? scores.reduce((acc, s) => acc + (s.qa_score || 0), 0) / scores.length : 0;
  const iepcMedia =
    scores.length > 0 ? scores.reduce((acc, s) => acc + (s.iepc_score || 0), 0) / scores.length : 0;
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              {
                label: 'QA Ciclo',
                value: currentScore ? `${(currentScore.qa_score || 0).toFixed(1)}%` : '—',
                color: currentScore?.qa_score >= 85 ? '#22C55E' : '#F59E0B',
              },
              {
                label: 'IEPC Ciclo',
                value: currentScore ? `${(currentScore.iepc_score || 0).toFixed(1)}%` : '—',
                color: currentScore?.iepc_score >= 85 ? '#22C55E' : '#F59E0B',
              },
              {
                label: 'NCs Total',
                value: String(ncs.length),
                color: ncs.length > 5 ? '#EF4444' : '#94A3B8',
              },
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p
                        className="text-xs font-semibold"
                        style={{ color: (score.qa_score || 0) >= 85 ? '#22C55E' : '#F59E0B' }}
                      >
                    ...
                    <div className="text-right">
                      <p
                        className="text-xs font-semibold"
                        style={{ color: (score.iepc_score || 0) >= 85 ? '#22C55E' : '#F59E0B' }}
                      >
...
export function AvaliacaoLevel({
  avaliacao,
  analista,
  onSelectAtendimento,
  onBack,
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        >
          <p
            ...
            style={{ color: avaliacao.qa >= 85 ? '#22C55E' : '#F59E0B' }}
          >
            {avaliacao.qa.toFixed(1)}%
          </p>
        ...
        >
          <p
            className="text-2xl font-bold"
            style={{ color: avaliacao.iepc >= 85 ? '#22C55E' : '#F59E0B' }}
          >
            {avaliacao.iepc.toFixed(1)}%
          </p>
          <p className="text-xs mt-1" style={{ color: '#94A3B8' }}>
            IEPC Médio
          </p>
              ...
              >
                <div>
                  <p className="text-sm font-medium text-white">
                    {at.protocolo || at.atendimento_id || `Atendimento ${idx + 1}`}
                  </p>
                  <p className="text-xs" style={{ color: '#94A3B8' }}>
                    {at.data_avaliacao
                      ? new Date(at.data_avaliacao).toLocaleDateString('pt-BR')
                      : avaliacao.periodo}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <span
                    className="text-xs font-semibold"
                    style={{ color: (at.qa_score || 0) >= 85 ? '#22C55E' : '#F59E0B' }}
                  >
...
export function AtendimentoLevel({
  atendimento,
  analista,
  avaliacao,
  onViewCriterios,
  onBack,
}: AtendimentoLevelProps) {
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              {
                label: 'IEPC Score',
                value: record ? `${(record.iepc_score || 0).toFixed(1)}%` : '—',
              },
    ...
    const load = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          const criteriosList = [];
          if (data.postura_etica !== undefined)
            criteriosList.push({ nome: 'Postura e Ética', score: data.postura_etica, peso: 20 });
          if (data.acuracidade_tecnica !== undefined)
            criteriosList.push({
              nome: 'Acuracidade Técnica',
              score: data.acuracidade_tecnica,
              peso: 25,
            });
          if (data.rastreabilidade !== undefined)
            criteriosList.push({ nome: 'Rastreabilidade', score: data.rastreabilidade, peso: 20 });
          if (data.fluxo_operacional !== undefined)
            criteriosList.push({
              nome: 'Fluxo Operacional',
              score: data.fluxo_operacional,
              peso: 20,
            });
          if (data.seguranca_informacao !== undefined)
            criteriosList.push({
              nome: 'Segurança da Informação',
              score: data.seguranca_informacao,
              peso: 15,
            });
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
                ...
                {score < 70 && (
                  <div className="flex items-center gap-1.5 mt-2">
                    ...
                    <p className="text-xs" style={{ color: '#EF4444' }}>
                      Abaixo do mínimo — requer atenção
                    </p>
                  </div>
                )}
              </div>
          ...
          {criterios.length === 0 && (
            <p className="text-center py-8 text-sm" style={{ color: '#94A3B8' }}>
              Nenhum critério disponível para este atendimento
            </p>
          )}
        </div>
      )}
      ...
      >
        ...
        <div className="p-5 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 120px)' }}>
          {state.level === 'ciclo' && state.ciclo && (
            <CicloLevel
              ciclo={state.ciclo}
              onSelectAnalista={(analista) =>
                setState((prev) => ({ ...prev, level: 'analista', analista }))
              }
          ...
          {state.level === 'analista' && state.analista && state.ciclo && (
            <AnalistaLevel
              ...
              ciclo={state.ciclo}
              onSelectAvaliacao={(avaliacao) =>
                setState((prev) => ({ ...prev, level: 'avaliacao', avaliacao }))
              }
          ...
          {state.level === 'avaliacao' && state.avaliacao && state.analista && (
            <AvaliacaoLevel
              ...
              onSelectAtendimento={(atendimento) =>
                setState((prev) => ({ ...prev, level: 'atendimento', atendimento }))
              }
              onBack={() => setState((prev) => ({ ...prev, level: 'analista' }))}
            />
          )}
          {state.level === 'atendimento' &&
            state.atendimento &&
            state.analista &&
            state.avaliacao && (
              <AtendimentoLevel
