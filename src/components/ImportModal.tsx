...
import { X, Upload, CheckCircle, Loader2, AlertCircle, ChevronRight, FileSpreadsheet,  } from 'lucide-react';
...
export default function ImportModal({ isOpen, onClose, onImportSuccess }: ImportModalProps) {
  ...
  useEffect(() => {
    if (!periodo) return;
    isCycleClosed(periodo)
      ? setCycleClosedWarning(true)
      : setCycleClosedWarning(false);
  }, [periodo]);
  ...
  const handleProcessWorkbook = async (
    ...
    let parsedScores = parseQAScoresCSV(scoresRows, periodo);
    let parsedNCs = parseNCsCSV(ncRows, periodo);
            ...
            <div className="space-y-4">
              ...
              <div
                ...
                className={`border-2 border-dashed rounded-md p-8 text-center transition-colors cursor-pointer ${
                  isDragging
                    ? 'border-blue-600 bg-blue-50/50' :'border-slate-300 hover:border-blue-500 bg-slate-50/50'
                }`}
                onClick={() => {
