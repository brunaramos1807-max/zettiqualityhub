...
import { X, Upload, CheckCircle, Loader2, AlertCircle, ChevronRight, Database, Info, ShieldCheck, BarChart2, TrendingUp, Hash } from 'lucide-react';
...
type ImportStep = 'choose-type' | 'upload' | 'validate' | 'done';
...
const FILE_TYPE_OPTIONS: { type: FileType; label: string; description: string; color: string; required?: boolean }[] = [
  {
    type: 'scores',
    label: 'Qualidade (QA/IEPC)',
    description: 'Pontuações de qualidade e IEPC por analista',
    color: '#1E40AF',
    required: true,
  },
  {
    ...
    description: 'Registros de NCs com tipo e pontos deduzidos',
    color: '#DC2626',
  },
  {
    type: 'elogios',
    label: 'Elogios',
    description: 'Elogios recebidos pelos analistas',
    color: '#16A34A',
  },
];

// ─── Schema definitions based on actual CSV files ────────────────────────────
...
function validateColumns(rows: Record<string, any>[], type: FileType): { valid: boolean; missing: string[]; warnings: string[] } {
  ...
  // Case-insensitive match after cleaning BOM/whitespace
  const missing = required.filter(
    (col) => !fileColumns.some(
      (fc) => fc.toLowerCase() === col.toLowerCase()
    )
  );
  ...
  if (!hasPeriod) {
    warnings.push('Coluna "Período" não encontrada — o período será definido pelo campo acima.');
  }
...
async function parseFile(file: File): Promise<Record<string, any>[]> {
  ...
  if (ext === 'csv') {
    return new Promise((resolve, reject) => {
      // First read raw text to detect delimiter
      const reader = new FileReader();
      reader.onload = (e) => {
        const text = (e.target?.result as string) || '';
        // Detect delimiter: count semicolons vs commas in the first line
...
function buildValidationSummary(
  ...
  if (type === 'ncs') {
    ...
    return {
      totalRows: parsedNCs.length,
      qaAvg: null,
      iepcAvg: null,
      totalNCs: parsedNCs.length,
      ncRows: parsedNCs.length,
      ncTotalPontos,
      elogioRows: null,
      parsedScores: [],
      parsedNCs,
  ...
  // elogios
  return {
    totalRows: parsedElogios.length,
    qaAvg: null,
    iepcAvg: null,
    totalNCs: null,
    ncRows: null,
    ncTotalPontos: null,
    elogioRows: parsedElogios.length,
    parsedScores: [],
    parsedNCs: [],
    parsedElogios,
  };
...
export default function ImportModal({ isOpen, onClose, onImportSuccess }: ImportModalProps) {
  ...
  const handleFileForType = async (file: File, type: FileType) => {
    ...
    try {
      ...
      if (validation.warnings.length > 0) {
        toast.warning('Aviso de importação', { description: validation.warnings[0] });
      }
    } catch (err: any) {
  ...
  // Called when user clicks "Importar" — parse and show validation summary
  const handleProceedToValidate = () => {
    if (!fileEntry || fileEntry.rows.length === 0 || !selectedType) {
      toast.error('Nenhum arquivo carregado');
      return;
    }
    const summary = buildValidationSummary(selectedType, fileEntry.rows, periodo);
    if (summary.totalRows === 0) {
      setImportError('Nenhum registro válido encontrado. Verifique se as colunas obrigatórias estão preenchidas.');
      return;
    }
    setImportError(null);
  ...
  // Called when user confirms on the validation screen
  const handleConfirmImport = async () => {
    ...
    try {
      const { parsedScores, parsedNCs, parsedElogios } = validationSummary;

      // Save to localStorage (primary local store)
      ...
      window.dispatchEvent(new CustomEvent('zetti_import_done', { detail: { tipo: tipoLabel, periodo, count } }));

      toast.success('Dados importados com sucesso!', {
        description: `${count} registros de ${selectedTypeInfo?.label} — Ciclo ${periodo}`,
      });
      onImportSuccess?.();
    } catch (err: any) {
        ...
        >
          <div>
            ...
            <p className="text-sm mt-0.5 text-slate-500">
              {step === 'choose-type' ? 'Selecione o tipo de planilha antes de importar'
        ...
        <div className="p-6 space-y-4">
          ...
          {step === 'choose-type' && (
            <>
                ...
                <div className="space-y-2">
                  {FILE_TYPE_OPTIONS.map((opt) => (
                    <button
                      ...
                      style={{
                        border: `2px solid ${selectedType === opt.type ? opt.color : '#E2E8F0'}`,
                        backgroundColor: selectedType === opt.type ? `${opt.color}08` : '#FAFAFA',
                      }}
                    >
                      <div
                      ...
                      {selectedType === opt.type && (
                        <CheckCircle size={16} style={{ color: opt.color }} />
                      )}
                    </button>
                  ))}
                </div>
              ...
              <button
                onClick={() => selectedType && setStep('upload')}
          ...
          {step === 'upload' && (
            <>
              <div className="flex items-center gap-2 mb-2">
                <button
                  onClick={() => { setStep('choose-type'); setFileEntry(null); }}
              ...
              >
                {fileEntry?.status === 'error' ? (
                  <>
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); setFileEntry(null); }}
                  ...
                  <>
                    ...
                    {fileEntry.validationWarnings && fileEntry.validationWarnings.length > 0 && (
                      <div className="w-full p-2 rounded-lg" style={{ backgroundColor: '#FFFBEB', border: '1px solid #FDE68A' }}>
                        {fileEntry.validationWarnings.map((w, i) => (
                  ...
                  <>
                    <div
                      ...
                      style={{ backgroundColor: `${selectedTypeInfo?.color}12` }}
                    >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {validationSummary.qaAvg !== null && (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#1E40AF' }}>
                        Média QA
                      </p>
                    </div>
                    <p className="text-2xl font-bold" style={{ color: '#1E3A8A' }}>
                      {fmt(validationSummary.qaAvg)}
                    </p>
                    <p className="text-xs mt-0.5" style={{ color: '#3B82F6' }}>calculado da planilha</p>
                  </div>
                )}

                {/* IEPC Avg — scores only */}
                {validationSummary.iepcAvg !== null && (
                  ...
                  >
                    ...
                    <p className="text-2xl font-bold" style={{ color: '#14532D' }}>
                      {fmt(validationSummary.iepcAvg)}
                    </p>
                    <p className="text-xs mt-0.5" style={{ color: '#22C55E' }}>calculado da planilha</p>
                  </div>
                )}

                {/* Total NCs from scores */}
                ...
                {validationSummary.ncTotalPontos !== null && (
                  ...
                  >
                    ...
                    <p className="text-2xl font-bold" style={{ color: '#881337' }}>
                      {validationSummary.ncTotalPontos}
                    </p>
              ...
              >
                ...
                <p className="text-xs text-amber-800">
                  Os valores acima foram <strong>calculados diretamente da planilha</strong>. Confirme se correspondem ao esperado antes de salvar. Ao confirmar, os dados existentes para o ciclo <strong>{periodo}</strong> serão substituídos.
                </p>
              </div>
            ...
            <div className="text-center py-4 space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  {validationSummary?.totalRows ?? fileEntry?.rows.length} registros de{' '}
                  <strong>{selectedTypeInfo?.label}</strong> importados para o ciclo{' '}
              ...
              <p className="text-xs text-slate-400">
                💡 Dados armazenados localmente. Limite aproximado: 5MB
              </p>
            </div>
          )}
        </div>
